	...
	/*for (size_t i = 0; i < this->cluster.size(); i++) {
		...
		for (size_t j = 0; j < individuos; j++) {
			...
			for (size_t k = 0; k < individuos; k++) {
				for (size_t t = 0; t < tabela.size(); t++) {
					d += (tabela[t](k, j) * pow(this->U[k][i], m) * this->coeficiente[t]);
				}
			}
			v.push_back(mp(d,j));
		}
	...
	for (size_t i = 0; i < individuos; i++) {
		for (size_t j = 0; j < this->cluster.size(); j++) {
			...
			for (size_t t = 0; t < tabela.size(); t++) {
				...
				for(size_t q = 0; q < Q; q++) {
					dist[j] += tabela[t](i, this->cluster[j].prototipo[t][q]) * this->coeficiente[t];
				}
			}
		}
...
double Resultado::atualizaJ(const Repositorio& repositorio) {
	...
	for (size_t i = 0; i < this->cluster.size(); i++) {
		for (size_t j = 0; j < individuos; j++) {
			double dist = 0;
			for (size_t t = 0; t < tabela.size(); t++) {
				// dist += (this->cluster[i].distancia(j, tabela[t]) * this->coeficiente[t]);
				//dist += (this->cluster[i].distancia(j, tabela[t]) * this->coeficiente[t]);
				//for(size_t ind = 0; ind < individuos; ind++) {
				//	dist += tabela[t](ind,this->cluster[i].prototipo[t]) * pow(this->U[ind][i],m) * this->coeficiente[t];
				//}
			}
...
void Resultado::atualizaOverallPrototype(const Repositorio& repositorio) {
	...
	for (size_t t = 0; t < tabela.size(); t++) {
		...
		for(size_t p = 0; p < individuos; p++) {
			...
			for (size_t i = 0; i < this->cluster.size(); i++) {
				for (size_t j = 0; j < individuos; j++) {
					dist += tabela[t](j, p) * pow(this->U[j][i], m) * this->coeficiente[t];
				}
			}
			if (cmp(dist, dmin) < 0) {
				dmin = dist;
...
double Resultado::calculaT(const Repositorio& repositorio, int k, int t, Array<int> prototipo) {
	...
	double T = 0;
	for(size_t i = 0; i < individuos; i++) {
		T += tabela[t](i,prototipo[t]) * pow(this->U[i][k],m) * this->coeficiente[t];
	}
	return T;
}
...
double Resultado::calculaJ(const Repositorio& repositorio, int k, int t) {
	...
	for(size_t i = 0; i < individuos; i++) {
		for(size_t q = 0; q < Q; q++) {
			J += tabela[t](i,this->cluster[k].prototipo[t][q]) * pow(this->U[i][k],m) * this->coeficiente[t];
		}
	}
	return J;
}
