...
Algoritmo::Algoritmo(size_t inicializacoes, size_t clusters, size_t m, size_t q, size_t limite, const Repositorio& repositorio, ostream& out) :
	inicializacoes(inicializacoes), clusters(clusters), repositorio(repositorio), out(out) {
	melhor = atual = Resultado(clusters, repositorio.tabela.size(), repositorio.tabela[0].n, m,q, 0);
...
void Algoritmo::executar() {
	...
	this->out << "# Coeficientes associado as tabelas:\n";
	this->out << fixed << setprecision(6);
	for (size_t i = 0; i < this->melhor.coeficiente.size(); i++) {
		this->out << "table[" << i << "] : " << this->melhor.coeficiente[i] << "\n";
	}
	this->out << "\n";
...
void Algoritmo::imprimirMatriz(ostream &out) {
	...
	//--------------------------------------------------------------------------------
	out << "\n# error: " << fixed << setprecision(2) << (100 * calculaErro(
			individuos, table)) << "%\n";
