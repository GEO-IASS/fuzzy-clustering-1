...
Resultado::Resultado(size_t k, size_t p, size_t t, size_t n, size_t m, size_t s, int id, double J, double CR) {
		...
		for (size_t j = 0; j < individuos; j++) {
			...
			for (size_t k = 0; k < individuos; k++) {
				for (size_t t = 0; t < tabela.size(); t++) {
					// TODO
					d += (tabela[t](k, j) * pow(this->U[k][i], m) * pow(this->coeficiente[i][t],s)); // TODO
				}
			}
			v.push_back(mp(d,j));
		}
...
bool Resultado::atualizaCoeficiente(const Repositorio& repositorio) {
	...
	for(size_t i = 0; i < this->cluster.size(); i++) {
		...
		for (size_t j = 0; j < tabela.size(); j++) {
			assert(cmp(den[j]));
			double tot = 0;
			for(size_t k = 0; k < tabela.size(); k++) {
				tot += pow( (den[j])/(den[k]), 1.0 / (s - 1));
			}
			assert(cmp(tot));
...
void Resultado::atualizarU(const Repositorio& repositorio) {
	...
	for (size_t i = 0; i < individuos; i++) {
		for (size_t j = 0; j < this->cluster.size(); j++) {
			...
			for (size_t t = 0; t < tabela.size(); t++) {
				// TODO
				dist[j] += (this->cluster[j].distancia(i, tabela[t]) * pow(this->coeficiente[j][t],s)); // TODO
			}
		}
...
double Resultado::atualizaJ(const Repositorio& repositorio) {
	...
	for (size_t i = 0; i < this->cluster.size(); i++) {
		for (size_t j = 0; j < individuos; j++) {
			...
			for (size_t t = 0; t < tabela.size(); t++) {
				// TODO
				dist += (this->cluster[i].distancia(j, tabela[t]) * pow(this->coeficiente[i][t],s)); // TODO
			}
