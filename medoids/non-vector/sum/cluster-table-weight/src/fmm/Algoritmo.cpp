...
Algoritmo::Algoritmo(size_t inicializacoes, size_t clusters, size_t prototipos, size_t m, size_t limite, size_t s, const Repositorio& repositorio, ostream& out) :
	inicializacoes(inicializacoes), clusters(clusters), prototipos(prototipos), repositorio(repositorio), out(out) {
	melhor = atual = Resultado(clusters, prototipos, repositorio.tabela.size(), repositorio.tabela[0].n, m, s, 0);
